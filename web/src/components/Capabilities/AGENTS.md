...
| `PlacementTargetsPanel.tsx` | Collapsed-by-default Placement Targets manager. Its summary shows target/runtime counts; expanding reveals the actual Claude/Codex MCP config destination and runtime skill directory, while cold storage shows its storage path. Auto-sets `teamShared` when kind is `project`. |
