...
| `PlacementTargetsPanel.tsx` | Inline Placement Targets manager. Runtime is explicit in the add form and every row shows both the actual Claude/Codex MCP config destination and runtime skill directory; cold storage shows its storage path. Auto-sets `teamShared` when kind is `project`. |
