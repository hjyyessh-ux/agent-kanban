...
export function InventoryView({
  ...
  const [selectedMcp, setSelectedMcp] = useState<McpInventoryItem | null>(null);
  ...
  const handleFreezeSkill = async (skill: DiscoveredSkill & SkillVisibility) => {
    ...
    setFreezingIds((prev) => new Set(prev).add(skill.id));
    setFreezeErrors((prev) => { const n = { ...prev }; delete n[skill.id]; return n; });
    try {
      await freezeSkillApi(skill.id);
      await onRefreshSkills();
    } catch (e: unknown) {
      setFreezeErrors((prev) => ({ ...prev, [skill.id]: e instanceof Error ? e.message : 'Freeze failed' }));
    } finally {
      setFreezingIds((prev) => { const n = new Set(prev); n.delete(skill.id); return n; });
    }
  };

  const handleFreezeMcpPlacement = async (item: McpInventoryItem, placement: McpPlacement, idx: number) => {
    if (placement.scope === 'project') {
      if (!window.confirm('project 항목: git 관리 파일(.mcp.json)이므로 변경이 발생합니다. 계속하시겠습니까?')) return;
    }
    const key = `mcp:${item.name}:${idx}`;
    const fromDir = placement.scope === 'local' || placement.scope === 'project'
      ? placement.dir
      : undefined;
    setFreezingIds((prev) => new Set(prev).add(key));
    setFreezeErrors((prev) => { const n = { ...prev }; delete n[`mcp:${item.name}`]; return n; });
    try {
      await freezeMcpApi(item.name, placement.scope, fromDir);
      await onRefreshSkills();
    } catch (e: unknown) {
      setFreezeErrors((prev) => ({ ...prev, [`mcp:${item.name}`]: e instanceof Error ? e.message : 'Freeze failed' }));
    } finally {
      ...
      <div className="cap-toolbar">
        ...
        <div className="cap-filter-group" role="group" aria-label="Filter by type">
          {(['all', 'mcp', 'skill'] as const).map((t) => (
            ...
            >
              {t === 'all' ? `All (${totalCount})` : t === 'mcp' ? `MCP (${mcp.length})` : `Skills (${skills.length})`}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="inv-section">
          ...
          <div className="inv-list">
            {filteredMcp.map((item) => {
              ...
              return (
                <div key={item.name} className="inv-item inv-item--mcp">
                  <div
                  ...
                  >
                    ...
                    <div className="inv-item-right">
                      ...
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); setSelectedMcp(item); }}
                      ...
                      <div key={i} className="inv-placement-row">
                        ...
                        {!p.managed && (
                          <button
                            ...
                            title={FREEZE_TITLE}
                            disabled={freezingIds.has(`mcp:${item.name}:${i}`)}
                            onClick={() => void handleFreezeMcpPlacement(item, p, i)}
                            aria-label={`Freeze ${item.name} (${p.scope}) to cold storage`}
                          >
                            {freezingIds.has(`mcp:${item.name}:${i}`) ? '…' : '❄ Freeze'}
                          </button>
                        )}
                      </div>
                    ))}
                  </div>
                  {freezeErrors[`mcp:${item.name}`] && (
                    <p className="inv-freeze-error">{freezeErrors[`mcp:${item.name}`]}</p>
                  )}
                </div>
              );
            })}
        ...
        <div className="inv-section">
          ...
          <div className="inv-list">
            {filteredSkills.map((skill) => {
              ...
              const isVisExpanded = expandedVisSkillId === skill.id;
              return (
                <div
                  key={skill.id}
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          setExpandedVisSkillId(isVisExpanded ? null : skill.id)
                        }
                    ...
                    <div className="inv-placement-row">
                      ...
                      {skill.scope !== 'system' && (
                        <button
                          ...
                          className="inv-freeze-btn"
                          title={FREEZE_TITLE}
                          disabled={freezingIds.has(skill.id)}
                          onClick={() => void handleFreezeSkill(skill)}
                          aria-label={`Freeze ${skill.skillName} to cold storage`}
                        >
                          {freezingIds.has(skill.id) ? '…' : '❄ Freeze'}
                        </button>
                      )}
                    </div>
                  </div>
                  {freezeErrors[skill.id] && (
                    <p className="inv-freeze-error">{freezeErrors[skill.id]}</p>
                  )}
                  {isVisExpanded && (
