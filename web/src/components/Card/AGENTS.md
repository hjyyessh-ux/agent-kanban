...
| File | Description |
|------|-------------|
| `CardDetailDialog.tsx` | Full detail/edit view for an existing card: `getDefaultCollapsedPhases()` opens the Prompt phase while the card is still open (`todo`/`in_progress`) and clamps it once finished (`complete`/`done`), where the Result is what gets read; inline field editing, runtime metadata, Script execution provenance/status, screenshots, queue settings, session resume, and question/feedback panels. Largest file in the component tree. |
...
| `card-result.ts` | `splitCardResult()` — peels `finalResult` (the run's last assistant text block) off the end of `result` (every block, joined) so the Result phase can open on the deliverable and fold the interim output above it. A `result` that no longer ends with `finalResult` (hand-edited, or rewritten by `kanban_update`) is treated as final in full. |
