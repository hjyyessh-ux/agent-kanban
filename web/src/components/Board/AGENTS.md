...
| `BoardCompleteSessionView.tsx` | Groups `done`/`complete` cards into collapsible session groups (`CompleteSessionGroup`), used instead of the flat card list for finished work (always for `done`; for `complete` only while the "Session 모아보기" toggle is on). A turn row's Result line summarizes `splitCardResult(card).final`, so it shows the deliverable rather than the interim output that preceded it. A feedback card's turn row is summarized by its instruction — `stripFeedbackHeader()` peels the generated `[Feedback for: …]` block first, which would otherwise fill the whole row (`board-selectors.ts`'s `getBoardSummary` does the same for board/list cards). An expanded group shows the newest `INITIAL_VISIBLE_TURNS` (5) turns and folds the older ones behind a `Show N more` button — the slice is taken on the newest-first array, before the per-group sort toggle reverses it, so it stays the newest five in either direction. |
