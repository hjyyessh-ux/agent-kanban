...
- Works 기본 화면은 미배정 세션을 가로 폭 전체로 먼저 보여주고, 그 아래에 진행 중 Work 목록을 같은 폭으로 배치한다. 미배정 목록의 높이를 제한해 작업 목록이 과도하게 밀리지 않게 한다. 상세의 일정·디렉토리는 Summary 위에 펼쳐 두고, 세션 선택과 관리 액션은 별도 펼치기 없이 노출한다. 활동·실행 결과는 연결된 세션의 대화에서 확인하며, Wiki 디렉토리·문서 목록은 일괄 보관 이후에만 보여준다.
