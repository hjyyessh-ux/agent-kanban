...
| Change Work lifecycle stamps (`resolvedAt`/`resolution`/`startedAt`) | `work-store.ts` | Terminal transitions auto-stamp `resolvedAt` + `resolution` (`done`→`completed`, `discarded`→`abandoned`); returning to `active` clears both; only the *first* session link back-dates `startedAt`. Card-level side effects live in `plugin/works/work-lifecycle.ts` — see `docs/invariants.md` |
| Change Work date validation | `work-store.ts` | `updateWork` rejects an edit that leaves `resolvedAt < startedAt`, but **only when the caller passed a date** — so a record already stored inverted can still be edited back into shape. The Timeline's bar-edge drag and the detail dialog's date inputs both land here |
