...
| `result-segments.ts` | `joinResultSegments()` — joins every streamed assistant text block into the card `result`; the CLI's own last-message is only a fallback. `lastResultSegment()` — the run's last text block, stored separately as the card's `finalResult` |
...
| `run-progress.ts` | `buildRunProgress()` (a run's events.jsonl) / `buildTranscriptProgress()` (a session transcript, for hook-minted cards with no run) → `CardRunProgress`. **Three** parsers, not two: claude stream-json, `codex exec --json` (`item.completed`, snake_case item types), and the interactive **codex rollout** (`event_msg`/`item_completed`, PascalCase item types). The rollout is a different wire format for the same facts, so feeding it to the exec parser yields zero steps rather than an error — `buildTranscriptProgress` takes the format from `locateSessionTranscript`, never from `card.agentRuntime` |
...
- "Complete" means **every assistant text block of the run, in order**, joined by `joinResultSegments()` (`result-segments.ts`). Both CLIs' own "final message" (Claude's `result` event, Codex's `-o` last-message file) carries only the LAST text; treat it strictly as a fallback for runs with no streamed text. Overwriting the accumulated segments with it dropped every mid-turn answer (`text → tool_use → text`) — regression tests: `claude-adapter.test.ts` "keeps every assistant text block", `codex-cli-adapter.test.ts` "keeps every agent message". Alongside it, `finalResult` carries only the **last** block (`lastResultSegment()`) so readers can separate the deliverable from the interim output that preceded it — both fields are written by the same `updateCard` call on every completion *and* failure path, so they never describe different runs.
