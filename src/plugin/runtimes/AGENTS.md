...
| `result-segments.ts` | `joinResultSegments()` — joins every streamed assistant text block into the card `result`; the CLI's own last-message is only a fallback |
