...
- Work grouping is **purely additive**: with no `WorkStore` injected, no `workIndex`, or a session that belongs to no `Work`, `groupCardsBySession` must behave exactly as it did per-session (a test pins `groupCardsBySession(cards, new Map())` equal to `groupCardsBySession(cards)`). Sessions of one Work collapse into a single `work:<id>` group whose document title is the **Work title**, overriding the LLM's suggestion; `sessionId`/`sessionTitle` stay unset and `sessionIds` (chronological) plus `workId` carry the provenance into the frontmatter. Cards of a `discarded` Work are retired as terminal `skipped` without any LLM call. Reading the `WorkStore` is best-effort — a failure falls back to session grouping rather than stalling the pipeline.
