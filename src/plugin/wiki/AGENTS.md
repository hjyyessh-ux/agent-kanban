...
| File | Description |
|------|-------------|
| `wiki-config.ts` | `WIKI_PROMPT_VERSION` (bump to invalidate old classifications and re-queue them as backfill targets), `WIKI_SETTING_KEYS`/`WIKI_SETTING_DEFAULTS` (`wiki.enabled` default `false`, `wiki.vault_dir` default blank, `wiki.model` default `gpt-5.5`, `wiki.route` default `codex`), and backward-compatible route inference for configs saved before `wiki.route` existed. |
| `wiki-llm.ts` | One-shot LLM runner abstraction (`WikiLlmRunner`). Production calls receive the persisted `codex`/`claude` route; the model-name resolver remains only as a backward-compatible fallback. `createClaudeWikiLlm()` spawns `claude -p --output-format json`; `createCodexWikiLlm()` spawns `codex exec --json -s read-only -c mcp_servers={}` (skips user MCP servers for speed); both set `WIKI_INTERNAL_ENV=1` so Claude/Codex `UserPromptSubmit` hooks skip card creation for these calls. |
...
| `wiki-transcript.ts` | `locateSessionTranscript()` / `loadSessionTranscript()` — best-effort enrichment for **both** CLI runtimes: `~/.claude/projects/<munged-dir>/<sessionId>.jsonl`, else the codex rollout `~/.codex/sessions/YYYY/MM/DD/rollout-*-<sessionId>.jsonl` found by glob. Existence decides, never `card.agentRuntime`, and the located `format` picks the parser. Returns `undefined` on any miss/error; head+tail truncated (4k/8k chars) |
| `wiki-worker.ts` | `WikiWorker` class — the orchestrator: `start()`/`stop()`/`restart()` (interval + reentrancy-guard reset), `kick()` (immediate pass), `backfill()`/`reprocess()` (re-queue via `store.markWikiPending`), `processQueue()` (reentrancy-guarded, groups pending cards, runs group workers up to `concurrency`), `getStatus()`/`getConfig()`/`saveConfig()` for the `/api/wiki/*` routes and Settings panel; `groupCardsBySession(cards, workIndex?)` is the exported grouping helper (session groups by default, one collapsed group per `Work` when a session is linked) |
...
- Work grouping is **purely additive**: with no `WorkStore` injected, no `workIndex`, or a session that belongs to no `Work`, `groupCardsBySession` must behave exactly as it did per-session (a test pins `groupCardsBySession(cards, new Map())` equal to `groupCardsBySession(cards)`). Sessions of one Work collapse into a single `work:<id>` group whose document title is the **Work title**, overriding the LLM's suggestion; `sessionId`/`sessionTitle` stay unset and `sessionIds` (chronological) plus `workId` carry the provenance into the frontmatter. A `discarded` Work is left out of the index entirely — discarding abandons the grouping only, so its sessions fall back to the per-session default and run the ordinary wiki flow (never a Work-state-driven skip). `active` Works **are** indexed on purpose (a card archived before completion still belongs to its Work's document), which is exactly what makes multi-batch Works normal — see the `workDocPath` rule below. Each index entry carries `wikiDocPath` for that reason. Reading the `WorkStore` is best-effort — a failure falls back to session grouping rather than stalling the pipeline.
- The Work header line in `buildGroupContext()` is emitted **only** for Work groups, so session-group prompts stay byte-identical and prior classifications remain valid. Changing the session-group prompt text still requires a `WIKI_PROMPT_VERSION` bump.
...
- Reprocessing overwrites the previous doc at its existing `docPath` instead of creating an orphaned duplicate — preserve this lookup when touching `processGroup()`. **A Work group asks `group.workDocPath` (= `Work.wikiDocPath`) first**, and only then falls back to `group.cards.find(c => c.wiki?.docPath)`. A Work's cards can reach the queue in more than one batch (a card archived while the Work was still `active`, the rest by the completion sweep), and the cards in the second batch carry no `docPath` from a batch they were not in — looking only at them wrote a *second* file under the same Work title. After a successful write the path is recorded back onto the Work (best-effort; a failure to remember it can only cost a duplicate later and must not fail a document that is already on disk).
