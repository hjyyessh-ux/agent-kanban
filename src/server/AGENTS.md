...
- Session aggregation lives in the exported `computeSessionAggregates(store, aggregateSessionsFn)` helper (`routes.ts`) so `GET /api/sessions` and `GET /api/works/inbox` share one view; add fields there, not inline in a route.
- `PATCH /api/works/:id` is not a plain store write: it goes through `applyWorkPatch()` (`src/plugin/works/work-lifecycle.ts`), which on `status: 'done'` flips every card of every linked session to `done` and archives them (handing them to the wiki pipeline). `works.done_confirm` gates that sweep behind a `confirmArchive: true` body flag — **do not add a second endpoint for it**; the Works UI calls this one path. `status: 'discarded'` records the resolution only. See `docs/invariants.md` → "Work 완료 라이프사이클".
...
- `POST /api/works/:id/sessions` resolves the session's earliest card `startedAt` (archived cards included) and passes it as a hint so the *first* link back-dates the Work's Timeline start.
